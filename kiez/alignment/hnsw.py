    ...
    def kneighbors(
        ...
        if query_points is None:
            raise NotImplementedError(
                "Please provide query_points to hnsw.kneighbors()."
            )
        ...
        neigh_ind_dist = self.index_.knnQueryBatch(
            query_points, k=n_candidates, num_threads=self.n_jobs
        )

        # If fewer candidates than required are found for a query,
        # we save index=-1 and distance=NaN
        n_test = query_points.shape[0]
        neigh_ind = -np.ones((n_test, n_candidates), dtype=np.int32)
        neigh_dist = np.empty_like(neigh_ind, dtype=query_points.dtype) * np.nan
